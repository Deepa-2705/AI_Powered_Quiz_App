...
export const OptionList = ({
  ...
  console.log("🔹 Correct Answer:", activeQuestion?.correctAnswer);

  const correctAnswerIndex =
    options && activeQuestion
      ? options.findIndex((option) => option === activeQuestion.correctAnswer)
      : -1; // Default to -1 if options or activeQuestion is undefined
  ...
  return (
    <div className="flex flex-col items-start space-y-1">
      {options && options.length > 0 ? (
        ...
        <p>Loading options...</p> // ✅ Prevents crashing
      )}
    </div>
  );  
};
